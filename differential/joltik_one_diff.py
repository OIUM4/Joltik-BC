...
class joltik_diff():
    ...
    def genConstraints_addition(self, file):
        constraints = list()
    ...
    def genModel(self, file, r):
        ...
        if os.path.exists(file):
            C = C + self.genConstraints_addition(file)

        V1 = getVariables_binary_Constraints(C)
        V2 = getVariables_General_Constraints(C)

        filename = 'joltik' + str(self.keysize) + '_diff_' + str(r) + '.lp'
        o = open(filename, 'w')
...
if __name__ == '__main__':
    ...
    file = f"joltik{keysize}_boom_{r}.sol"

    m = joltik_diff(keysize)

    m.genModel(file, r)
    m = read('joltik' + str(keysize) + '_diff_' + str(r) + '.lp')
    m.optimize()
    # for v in m.getVars():
    #     print(v.varName, v.x)

    m.write('joltik' + str(keysize) + '_diff_' + str(r) + ".sol")
